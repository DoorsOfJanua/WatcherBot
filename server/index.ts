  ...
  void (async () => {
    try {
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart WatcherBotRoom");
...
async function cloudRoutineReadiness(): Promise<{ ready: boolean; reason?: string }> {
  ...
  if (!instance) {
    return { ready: false, reason: "The Cloud VM runner is unavailable. Restart WatcherBotRoom and try again." };
  }
  try {
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `WatcherBot Room credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `WatcherBot Room credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.bot.busy) {
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/ask-bot") {
        ...
        const prefixed = `[Message from @${currentFrom.name}, another bot in this WatcherBot Room workspace. Reply to them.]\n\n${message}`;

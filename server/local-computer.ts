...
export function decodeLinuxDescriptor(value: LinuxConnectionDescriptor): LocalComputerConnection | null {
  ...
  if (
    ...
    (mcp.env as Record<string, unknown>).CUA_DRIVER_HOST_BUNDLE_ID !== "com.watcherbotroom.app" ||
...
export function readCuaConnection({
  ...
  if (platform === "darwin") {
    ...
    for (const directory of ["WatcherBot Room", "watcherbotroom", "OpenGrokBot", "opengrokbot"]) {

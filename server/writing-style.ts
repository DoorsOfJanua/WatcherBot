...
/** A small style-only block for every engine, including remote engines. */
export function writingStyleSystemPrompt(botId: string): string {
  ...
  return [
    ...
    average
      ? `Observed sentence length after edits: about ${average} words on average. Treat this as a tendency, not a rule.`
      : "",

...
//
// WatcherBot Room owns only the sandbox boundary: image preparation, container
...
export const MANAGED_LABEL = "com.watcherbotroom.local-vm";
export const DRIVER_LABEL = "com.watcherbotroom.cua-driver";
export const BASE_IMAGE_LABEL = "com.watcherbotroom.cua-base";
export const WORKSPACE_LABEL = "com.watcherbotroom.workspace";
export const TARGET_LABEL = "com.watcherbotroom.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/watcherbotroom-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/watcherbotroom/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/start-watcherbotroom-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-watcherbotroom-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:watcherbotroom-cua-driver]' \\
      'command=/usr/local/bin/start-watcherbotroom-cua-driver.sh' \\
      'user=cua' \\
...
export async function containerComputerStatus(
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/watcherbotroom-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "watcherbotroom-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/watcherbotroom-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use WatcherBot Room's high-latency REST adapter. Local VMs

...
export interface MissionDispatcherOptions {
  ...
  execute: (
    ...
    item: WorkItem,
    agentId: string,
  ) => Promise<MissionExecutionResult> | MissionExecutionResult;
}
  ...
  async runOnce(): Promise<MissionDispatchOutcome[]> {
    ...
    for (const mission of this.manager.list()) {
      ...
      for (const item of this.manager.readyItems(mission.id, this.now())) {
        ...
        if (claimedAgents.has(agentId) || !this.canExecute(mission, item, agentId)) continue;
    ...
    const settled = await Promise.allSettled(claimed.map(async (dispatch): Promise<MissionDispatchOutcome> => {
      ...
      const heartbeat = setInterval(() => {
        try {
          this.manager.renewClaim(dispatch.missionId, dispatch.itemId, dispatch.agentId, this.now());
        } catch {
          // Settlement or restart owns recovery.
        }
      }, this.heartbeatMs);
      heartbeat.unref?.();
      try {
        ...
        try {
          this.manager.failWorkItem(dispatch.missionId, dispatch.itemId, reason);
        } catch {
          // The lease or mission may have changed while execution settled.
        }
    ...
    return settled.map((entry, index) => entry.status === "fulfilled"
      ? entry.value
      : { ...claimed[index]!, status: "fail", error: String(entry.reason) });
  }

  tick(): Promise<MissionDispatchOutcome[]> {
    return this.runOnce();
  }
}

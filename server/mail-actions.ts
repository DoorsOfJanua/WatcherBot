...
// after claim is deliberately not retryable: acceptance can be ambiguous,
// and duplicate mail is worse than a visible manual check.
...
function normalizeRecipients(value: unknown, name: string): string[] {
  ...
  if (result.some((item) => !EMAIL.test(item))) {
...
export function normalizeMailDraft(value: unknown): MailDraft {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("mail draft must be a JSON object");
  }
...
/** The exact bytes hashed by this harness and the provider gate. */
...
export class MailActionStore {
      ...
      if (Array.isArray(raw)) {
        for (const value of raw) {
          ...
          try {
            ...
            // Fail closed on a malformed or tampered payload.
          }
        }
      }
    } catch {
      // First run or malformed legacy file.
    }
  }
  ...
  update(
    receiptId: string,
    patch: Partial<Pick<FrozenMailAction, "state" | "updatedAt" | "providerReceipt" | "failure">>,
  ): FrozenMailAction {
...
export class MailActionCoordinator {
  ...
  stage(input: {
    ...
    draft: unknown;
    expiresInMs?: number;
  }): { action: FrozenMailAction; receipt: ActionReceipt } {
  ...
  /** A revision creates a new frozen receipt before closing the old one. */
  ...
  async approveAndSend(receiptId: string, approvedBy: string): Promise<FrozenMailAction> {
    ...
    // This synchronous claim is the at-most-once gate across desktop + phone.
...
/** Janua's proven local AgentHQ gateway, with overrides for moving it. */
export function januaMailGatewayConfig(): GatewayConfig {
  return {
    pythonPath: envPath(
      "OMB_MAIL_PYTHON",
      existsSync("/opt/anaconda3/bin/python3") ? "/opt/anaconda3/bin/python3" : "/opt/homebrew/bin/python3",
    ),
...
export class PythonMailGateway implements MailSender {
  ...
  async send(draft: MailDraft, expectedHash: string): Promise<MailSendReceipt> {
    ...
    if (mailDraftHash(normalized) !== expectedHash) {
      throw new Error("Email draft hash changed after approval; refusing to send");
    }
    ...
    const output = await new Promise<string>((resolve, reject) => {
      ...
      child.once("close", (code) => finish(
        code === 0 ? undefined : new Error(safeFailure(stderr) || `Mail gateway failed with code ${code ?? -1}`),
      ));

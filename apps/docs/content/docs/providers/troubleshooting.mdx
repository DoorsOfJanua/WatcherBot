...
3. Restart WatcherBot Room after installation.
...
Run the provider's own status or login command in a terminal. WatcherBot Room uses the CLI's authenticated session; installing the binary alone is not enough.

A `401 Unauthorized` response from `api.openai.com` means the launched Codex process did not receive valid authentication. Confirm `codex login status`, sign in again if needed, then restart WatcherBot Room. Do not paste a ChatGPT password into WatcherBot Room.
...
- Try the provider CLI directly with the same model to separate an account problem from an WatcherBot Room integration problem.

...
description: Know what WatcherBot Room stores locally and what to preserve.
icon: DatabaseBackup
---

WatcherBot Room keeps its application state under `~/.openmausbot`, including configuration, transcripts, message attachments, device pairing state, and the SQLite message store.

## Back up

Close WatcherBot Room before taking a filesystem-level backup so SQLite and configuration files are captured consistently. Preserve the entire `.openmausbot` directory rather than selecting individual database files.

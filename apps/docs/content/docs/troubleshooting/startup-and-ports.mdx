...
1. Quit every WatcherBot Room window.
2. Confirm no old WatcherBot Room process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/WatcherBot Room`. On Windows they are under the WatcherBot Room log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clean WatcherBot Room data directory. This is useful for reproduction and screenshots without touching a real workspace.

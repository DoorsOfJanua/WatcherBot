...
export interface MailPreviewMeta {
  to: string;
  subject: string;
}
...
export function mailPreviewMeta(detail: string): MailPreviewMeta {
  return {
    to: header(detail, "To"),
...
function mailItem(bot: Bot, message: Message): AttentionItem {
  const detail = message.card?.subtitle ?? "";
  const meta = mailPreviewMeta(detail);
  return {
...
function groupedMailItem(bot: Bot, children: AttentionItem[]): AttentionItem {
  ...
  const recipients = sorted
    .map((item) => item.summary.replace(/^To\s+/i, ""))
    .filter(Boolean);
  const named = recipients.slice(0, 3).join(", ");
  ...
  return {
    ...
    summary: named ? `${named}${more ? ` +${more} more` : ""}` : "Review Mailman's draft batch",
    at: sorted[0].at,
    actionable: true,
    children: sorted,
  };
...
/**
 * One normalized attention projection over the product's existing durable
 * sources. It deliberately does not persist another copy of approvals,
 * unread work, or routine failures: those records already own their truth.
 */
export function attentionItems(
  ...
  for (const bot of bots) {
    ...
    const mail = cards.filter((message) => message.card?.tool === "email.send").map((message) => mailItem(bot, message));

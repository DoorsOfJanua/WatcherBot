...
describe("attention projection", () => {
  it("extracts human mail labels and groups an exact-draft batch", () => {
  ...
  it("keeps local scope and prioritizes blocking cards", () => {
    const watcher = bot("watcher", [], { busy: true });
    const mailman = bot("mailman", [
      message("approval", { title: "Choose a direction", subtitle: "Which version?", options: ["A", "B"] }),
    ], { unread: true });
    expect(attentionItems({ bots: [watcher, mailman], routineRuns: [] }, "watcher").map((item) => item.botId))
      .toEqual(["watcher"]);
    expect(attentionItems({ bots: [watcher, mailman], routineRuns: [] })[0])
      .toMatchObject({ botId: "mailman", kind: "question", priority: "blocking" });
  });

  it("surfaces only unseen failed routines", () => {
    ...
    expect(attentionItems({ bots: [watcher], routineRuns: [failed] })[0])
      .toMatchObject({ kind: "routine", summary: "Calendar connection expired" });
    expect(attentionItems({ bots: [watcher], routineRuns: [{ ...failed, seenAt: 110 }] }))
      .toEqual([]);
  });
});

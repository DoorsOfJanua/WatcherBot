...
>(input: {
  ...
  if (action === "take" && syncNativeBrowser && !(await setNativeBrowserControl(true))) {
    throw new Error("WatcherBotRoom could not pause this bot's browser safely");
  }
  const snap = await requestControl(action);
  if (snap.held !== (action === "take")) {
    throw new Error(`WatcherBotRoom could not ${action === "take" ? "confirm" : "release"} computer control`);
  }

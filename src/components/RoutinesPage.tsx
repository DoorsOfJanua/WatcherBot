...
export function RoutinesPage() {
  ...
  const sectionMeta = section === "calendar"
    ? { icon: <CalendarDays size={21} className="text-accent" />, description: "Routines wake agents on schedule." }
    : section === "watchers"
      ? { icon: <Radar size={21} className="text-accent" />, description: "Watchers check quietly and return only when a source changes or needs attention." }
      : section === "missions"
        ? { icon: <Goal size={21} className="text-accent" />, description: "Missions keep multi-step outcome work moving under an accountable lead." }
      ...
      >
        ...
        <div className="mt-3 rounded-xl border border-hairline/45 bg-panel/70 px-3.5 py-2.5 text-[11.5px] leading-relaxed text-ink-secondary">
          {section === "calendar" ? (
            ...
            <><strong className="font-medium text-ink">Watcher</strong> = a quiet change detector. Unchanged checks stay silent.</>
          ) : section === "missions" ? (
            <><strong className="font-medium text-ink">Mission</strong> = a durable outcome with accountable work steps.</>
          ) : (

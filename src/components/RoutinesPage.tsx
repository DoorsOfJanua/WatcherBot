...
function scheduleLabel(routine: Routine) {
  ...
  if (routine.schedule.type === "interval") {
    const window =
      routine.schedule.start || routine.schedule.end
...
function projectedItems(routines: Routine[], runs: RoutineRun[], from: number, to: number): CalendarItem[] {
  ...
  for (const routine of routines) {
    ...
    for (let day = startOfDay(from); day < to; day = addDays(day, 1)) {
      ...
      // An interval routine would flood the grid with one chip per firing;
      // project a single chip at the day's first occurrence and let the run
      // receipts carry the real history.
      const at =
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {(windowStart || windowEnd) && (
                        <button type="button" onClick={() => { setWindowStart(""); setWindowEnd(""); }} className="text-[12px] text-ink-secondary hover:text-ink">All day</button>
                      )}
                    </div>
                  </>
                ) : (

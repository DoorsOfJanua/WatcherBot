...
export function CallTargetButton({
  ...
  const reason = !capabilitiesReady
    ...
    : !capabilities.dictation.available
      ? "Calls require the WatcherBotRoom desktop app on macOS because speech recognition runs on-device."
      : !window.ogb?.speechStart
        ? "The speech service is unavailable in this app build. Restart or update WatcherBotRoom."
        : !configured

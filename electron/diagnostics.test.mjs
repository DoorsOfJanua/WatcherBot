...
describe("buildDiagnosticsReport", () => {
  ...
  it("never includes an absolute log path in the report heading", () => {
    const report = buildDiagnosticsReport({
      ...
      logPath: "/Users/ada/Library/Logs/WatcherBotRoom/server.log",
    });
...
describe("diagnosticsFileName", () => {
  it("uses watcherbotroom-diagnostics-YYYYMMDD-HHmmss.txt", () => {
    expect(diagnosticsFileName(new Date(2026, 7, 22, 16, 5, 9))).toBe(
      "watcherbotroom-diagnostics-20260822-160509.txt",
    );
  });
});

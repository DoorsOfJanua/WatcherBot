import { app, BrowserWindow, clipboard, desktopCapturer, dialog, ipcMain, Menu, safeStorage, screen, session, shell, systemPreferences, utilityProcess } from "electron";
...
const { textContextMenuTemplate } = require("./context-menu.cjs");
...
// goes to server.log in the OS log dir (~/Library/Logs/WatcherBotRoom on macOS,
// Console.app-visible; %APPDATA%\WatcherBotRoom\logs on Windows), which is also
  ...
  for (let i = 0; i < 40; i++) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (body?.app === "watcherbotroom" && body.pid === proc.pid && body.static) return proc;
...
const ERROR_PAGE =
  ...
  encodeURIComponent(
    `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#070707;color:#fcfcfc;font:15px -apple-system,system-ui"><div style="text-align:center;max-width:360px"><div style="font-size:40px">👁️</div><h2 style="font-weight:600;margin:12px 0 6px">Couldn't start the bot server</h2><p style="color:#fcfcfc99;line-height:1.5">Something else is using its ports. Quit and reopen The WatcherBot — if it keeps happening, restart your computer.</p></div></body>`,
  );
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The WatcherBot window is unavailable");
  ...
  const viewer = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "watcherbotroom-desktop-viewer",
    },
  });
...
// live desktop inside WatcherBot Room instead of sending the person to a browser.

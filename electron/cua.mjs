...
async function startEmbedded(binary) {
  ...
  // Check them in Electron main so macOS evaluates the packaged app rather
  // than a terminal or helper-process identity. Startup and Retry must be safe
  // to repeat: preflight silently and leave prompting to a renderer gesture.

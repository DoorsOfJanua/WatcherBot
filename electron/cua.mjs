...
//    EmbeddedCuaDriverHost so TCC grants attribute to WatcherBot Room and the
//    driver inherits them. One prompt, named WatcherBot Room, out of the box.
...
const HOST_BUNDLE_ID = "com.watcherbotroom.app";
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed WatcherBot Room.

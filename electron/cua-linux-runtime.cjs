...
function createLinuxCuaRuntime({
  ...
  const runtimeRoot = () => {
    ...
    if (configured && path.isAbsolute(configured)) {
      try {
        ...
        if (
          ...
          const root = ensurePrivateDirectory(path.join(configured, "watcherbotroom-cua"));
    ...
    const currentUid = process.getuid?.() ?? os.userInfo().uid;
    const root = ensurePrivateDirectory(path.join(os.tmpdir(), `watcherbotroom-cua-${currentUid}`));
    cleanupStaleRuntimeDirectories(root);
    return root;
  };

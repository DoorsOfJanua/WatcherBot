#!/usr/bin/env node
// Model Context Protocol (MCP) Server for WatcherBotRoom
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid WatcherBotRoom URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("WatcherBotRoom URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("WatcherBotRoom URL must not contain credentials; use OPENMAUSBOT_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("WatcherBotRoom URL must be an origin without a path, query, or fragment");
  }
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`WatcherBotRoom API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`WatcherBotRoom API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "watcherbotroom") {
        failures.push(`${candidate} answered, but it was not WatcherBotRoom`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running WatcherBotRoom server. ${failures.join("; ")}`);
}
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the WatcherBotRoom server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid WatcherBotRoom ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "watcherbotroom") throw new Error("The configured endpoint is not a WatcherBotRoom server");
      return {
        status: "connected",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("WatcherBotRoom did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("WatcherBotRoom did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("WatcherBotRoom did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("WatcherBotRoom did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("WatcherBotRoom did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("WatcherBotRoom did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the WatcherBotRoom team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("WatcherBotRoom MCP server running on stdio");
}

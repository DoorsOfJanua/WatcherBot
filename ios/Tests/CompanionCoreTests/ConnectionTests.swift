...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        let token = "wtb_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string: "watcherbot://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Milind%27s%20Mac"))
    ...
    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "watcherbot://pair?address=mac.local&code=004209"))
        XCTAssertEqual(PairingInvite.parse(url)?.credential, "004209")
    }

    func testCarriesTheFallbackHostsFromTheInvite() throws {
        let url = try XCTUnwrap(URL(string:
            "watcherbot://pair?address=macbook.tail1234.ts.net%3A8810&code=004209" +
            "&hosts=macbook.tail1234.ts.net,192.168.1.42,watcherbot-aa.local"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["macbook.tail1234.ts.net", "192.168.1.42", "watcherbot-aa.local"])
    }
    ...
    func testDropsUnusableFallbackHostsWithoutRefusingTheInvite() throws {
        ...
        let url = try XCTUnwrap(URL(string:
            "watcherbot://pair?address=mac.local&code=004209&hosts=%20192.168.1.42%20,,bad%2Fslash,has%20space"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["192.168.1.42"])

        // and an invite with no usable candidate keeps the single address
        let empty = try XCTUnwrap(URL(string: "watcherbot://pair?address=mac.local&code=004209&hosts=bad%2Fslash"))
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "watcherbot://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "watcherbot://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "watcherbot://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "watcherbot://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "watcherbot://pair?address=one.local&address=two.local&code=123456"))))
    }

...
```sh
git clone https://github.com/DoorsOfJanua/WatcherBot
cd WatcherBot
...
dns-sd -B _watcherbot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/The WatcherBot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open WatcherBot.xcodeproj
```
...
```sh
xcodebuild -project WatcherBot.xcodeproj \
  -scheme WatcherBot \
  -sdk iphonesimulator \
...
```sh
xcodebuild -project WatcherBot.xcodeproj -scheme WatcherBot \
...
1. **Pair.** In The WatcherBot → Settings → Companion, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open The WatcherBot,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → The WatcherBot → Local Network. If the
...
3. **In The WatcherBot → Settings → Companion:** with the toggle on, the panel now
...
- **Xcode console**, subsystem `com.doorsofjanua.watcherbot`: `opening stream`,

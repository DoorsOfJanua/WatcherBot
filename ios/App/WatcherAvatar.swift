...
enum WatcherPalette {
    /// src/lib/mascot.ts — WATCHER_COLORS
...
enum WatcherSilhouette {
    ...
    static func fit(_ rect: CGRect) -> CGAffineTransform {
        let side = WatcherFaceData.faceBox + margin * 2
...
/// on the desktop. `WatcherFaceEngine` is a port of the frame loop in
/// `WatcherMascot/WatcherAvatar.tsx`: a state picks a pool of expressions and
...
struct WatcherAvatar: View {
    ...
    @State private var engine = WatcherFaceEngine()
...
final class WatcherFaceEngine {
    private(set) var state: WatcherState = .idle
    private var expression = 0
    private var currentRings: [[CGPoint]] = [WatcherFaceData.ring(0, eye: 0), WatcherFaceData.ring(0, eye: 1)]
    private var targetRings: [[CGPoint]] = [WatcherFaceData.ring(0, eye: 0), WatcherFaceData.ring(0, eye: 1)]
    private var currentMouth = WatcherFaceData.mouth(0)
    private var targetMouth = WatcherFaceData.mouth(0)
    private var currentGaze = WatcherFaceData.gaze(0)
    private var targetGaze = WatcherFaceData.gaze(0)
    ...
    func setState(_ new: WatcherState, now: Date) {
        guard !started || new != state else { return }
        started = true
        state = new
        stateStart = now
        select(WatcherFaceData.pools[new]?.first ?? 0)
        if last == nil { morph = 1; velocity = 0 } // first frame: rest on it, no morph in
        nextExpressionAt = schedule(WatcherFaceData.expressionCadence[new], from: now)
        nextBlinkAt = schedule(WatcherFaceData.blink[new], from: now)
    }
    ...
    private func select(_ index: Int) {
        let i = ((index % WatcherFaceData.expressionCount) + WatcherFaceData.expressionCount) % WatcherFaceData.expressionCount
        if i == expression && morph >= 1 { return }
        currentRings = displayedRings()
        currentMouth = displayedMouth()
        currentGaze = displayedGaze()
        targetRings = [WatcherFaceData.ring(i, eye: 0), WatcherFaceData.ring(i, eye: 1)]
        targetMouth = WatcherFaceData.mouth(i)
        targetGaze = WatcherFaceData.gaze(i)
        expression = i
        morph = 0
        velocity = 0
    }
    ...
    func step(now: Date) {
        ...
        if let due = nextExpressionAt, now >= due {
            let pool = WatcherFaceData.pools[state] ?? [0]
            let alternatives = pool.filter { $0 != expression }
            select(alternatives.randomElement() ?? pool[0])
            nextExpressionAt = schedule(WatcherFaceData.expressionCadence[state], from: now)
        }
        if let due = nextBlinkAt, now >= due {
            blinkStart = now
            nextBlinkAt = schedule(WatcherFaceData.blink[state], from: now)
        }
    }
    ...
    static let cometTrails: [WatcherState: CometSpec] = [
    ...
    func draw(in context: inout GraphicsContext, size: CGSize, color: String, bodyMotion: Bool, comets: Bool = false, at now: Date = Date()) {
        ...
        context.concatenate(WatcherSilhouette.fit(rect))
        ...
        let centre = CGPoint(x: WatcherFaceData.faceBox / 2, y: WatcherFaceData.faceBox / 2)
        ...
        if bodyMotion {
            bodyContext.concatenate(bodyTransform(WatcherFaceData.motion[state] ?? WatcherBodyMotion(), elapsed: elapsed))
        }
    ...
    private func drawBody(in context: inout GraphicsContext, color: String, now: Date) {
        let body = WatcherSilhouette.inFaceBox
        let bounds = WatcherSilhouette.faceBoxBounds
        context.fill(body, with: .linearGradient(
            Gradient(stops: WatcherPalette.gradientStops(color)),
        ...
        let a = WatcherFaceData.anchor
        context.translateBy(x: a.x, y: a.y)
        context.scaleBy(x: a.scale, y: a.scale)
        context.translateBy(x: -WatcherFaceData.faceCentre.x, y: -WatcherFaceData.faceCentre.y)
        ...
        context.stroke(
            ...
            with: .color(.white),
            style: StrokeStyle(lineWidth: WatcherFaceData.mouthStroke, lineCap: .round)
        )
    }

    /// The desktop's `bodyTransform`, in face-box units, elapsed in ms.
    private func bodyTransform(_ m: WatcherBodyMotion, elapsed: CGFloat) -> CGAffineTransform {
        let centre = WatcherFaceData.faceBox / 2
        let ground = WatcherFaceData.faceBox
...
/// with The WatcherBot's desktop roster.

...
struct UpdatesSheet: View {
    ...
    private func section(_ title: String, tint: Color?, kind: ChatUpdate.Kind) -> some View {
        ...
        if !items.isEmpty {
            let color = kind == .needsYou ? WatcherPalette.color(items[0].chat.color) : Color.secondary
    ...
    var body: some View {
        Button(action: open) {
            HStack(alignment: .top, spacing: 12) {
                ChatAvatarView(chat: update.chat, size: 40, state: WatcherState.forChat(update.chat, in: session.state))
                                        ...
                                        .background(
                                            Capsule().fill(
                                                CardStyle.isRefusal(option)
                                                    ...
                                                    : WatcherPalette.color(update.chat.color)
                                            )
                                        )
                                }
                                .buttonStyle(.plain)
                ...
                case .toReview:
                    HStack(spacing: 6) {
                        Circle().fill(WatcherPalette.color(update.chat.color)).frame(width: 10, height: 10)

...
public struct PairingInvite: Equatable, Sendable {
    ...
    public static func parse(_ url: URL) -> PairingInvite? {
        guard url.scheme?.lowercased() == "watcherbot",
    ...
    private static func credential(from values: [String: String]) -> String? {
        if let token = values["token"] {
            guard token.hasPrefix("wtb_pair_"),
                  token.utf8.count == 52,
                  token.dropFirst("wtb_pair_".count).utf8.allSatisfy({
...
public struct CompanionClient: Sendable {
    ...
    /// The WatcherBot → Settings → Companion"), so passing them through beats
    ...
    /// Make a persistent The WatcherBot teammate with the identity authored on

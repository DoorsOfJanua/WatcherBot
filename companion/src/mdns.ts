...
export interface ServiceInfo {
  ...
  name: string;
  /** e.g. "_watcherbot._tcp" */
  type: string;
  port: number;
  /** the name our A records claim, e.g. "watcherbot-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
export function dnsLabel(text: string, fallback = "The WatcherBot"): string {
  let label = text
    .replace(/[\u0000-\u001f\u007f]/g, "")
    .replace(/\./g, " ")
    .trim();
...
export function defaultHostName(machine = hostname()): string {
  ...
  return `watcherbot-${digest}.local`;
}

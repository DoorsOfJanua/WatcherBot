# Contributing to WatcherBot Room
...
```sh
git clone https://github.com/milind-soni/WatcherBot Room && cd WatcherBot Room
pnpm install
...
- stable `WatcherBot Room-amd64.deb` and `WatcherBot Room.AppImage` copies used by the latest-download links;
...
Data lives in `~/.myagent-room/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.myagent-room`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.myagent-room/config.json` via `PUT /api/config` and the API
